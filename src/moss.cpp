...
void print_help() {
    std::cout <<
              ...
              "       -n <germline VCF> -l <candidates.vcf> -o <output VCF>\n"
              "\n"
              ...
              "  -l, --loci <VCF>       candidate variants VCF file as generated by scripts/union_candidates.py\n"
              "\n"
              "General options\n"
...
int main(int argc, char **argv) {
    int c;
    const char *const short_opts = "b:R:r:o:n:l:t:m:B:M:d:h";
    ...
    std::cout << std::endl << "# Candidate variants file:\t";
    if (!loci_file.empty()) {
        if (!is_file_exist(loci_file)) {
            std::cerr << "\n Error: Failed to open candidates file " << loci_file << std::endl;
            return -1;
        } else {
            std::cout << loci_file << '\t';
        }
    ...
    if (loci_file.empty()) {
        std::cerr << std::endl
                  << "Error: No candidates file specified. Use `-l candidates.vcf` to specify."
                  << std::endl;
        return -1;
    }
    loci = moss::import_loci(loci_file);
    
    std::cout << "# Candidate variants imported" << std::endl;
